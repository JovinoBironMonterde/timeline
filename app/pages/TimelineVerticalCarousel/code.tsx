...
import { TimeLineDataWithList } from './../../components/TimelineData';

export default function TabsLayout({ children }: { children: React.ReactNode }) {
  const [active, setActive] = useState<'preview' | 'code' | 'data'>('preview');

  const dataCode = `export const TimeLineDataWithList = ${JSON.stringify(
    TimeLineDataWithList,
    null,
    2
  )};`;
      ...
      <div className="w-full Experience-Wrapper">
          ...
          <div className="w-full hidden sm:hidden xl:block">
            {TimelineData.map((timeline, index) => {
              ...
              return (
                <div key={timeline.id} className="w-full grid grid-cols-2 gap-50 p-10 items-start relative mb-10">
                  {/* Center Icon */}
